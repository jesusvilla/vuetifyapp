<template>
  <v-sidebar v-model="isActive" fixed="fixed">
  <div class="vuetify">
    <router-link class="sidebar__logo" to="/about"><img src="~public/v.png" height="100" width="100" alt="Vuetify Logo"/></router-link>
    <div class="title">Vuetify</div><a href="https://github.com/vuetifyjs/vuetify/releases/tag/v0.11.1" target="_blank">v0.11.1</a>
  </div>
  <v-divider light="light"></v-divider>
  <v-list dense="dense">
    <template v-for="item in items">
      <v-list-group v-if="item.items" v-bind:group="item.group">
        <v-list-tile slot="item" ripple="ripple">
          <v-list-tile-action>
        ...
        <v-list-item v-for="subItem in item.items">
          <v-list-tile :href="subItem.href" v-bind:router="!subItem.target" ripple="ripple" v-bind:disabled="subItem.disabled" v-bind:target="subItem.target">
            <v-list-tile-content>
              <v-list-tile-title>{{ subItem.title }}</v-list-tile-title>
            </v-list-tile-content>
            <v-list-tile-action v-if="subItem.action">
              <v-icon class="success--text">{{ subItem.action }}</v-icon>
            </v-list-tile-action>
          </v-list-tile>
        </v-list-item>
      </v-list-group>
      <v-subheader v-else-if="item.header">{{ item.header }}</v-subheader>
      <v-divider v-else-if="item.divider" light="light"></v-divider>
      <v-list-item v-else="v-else">
        <v-list-tile :href="item.href" router="router" ripple="ripple" v-bind:disabled="item.disabled">
          <v-list-tile-action>
            <v-icon>{{ item.action }}</v-icon>
          </v-list-tile-action>
          <v-list-tile-content>
